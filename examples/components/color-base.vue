<template>
  <div class="color-pain">
    <h3>概述</h3>
    <p>Best-UI 默认具有一套较为美观的配色方案，在开发中也推荐使用一下色板来达到视觉统一效果</p>
      ...
      <b-col span="8">
        <div class="color-wrap">
          <div
            class="item"
            :class="name"
            v-for="(name,index) in primaryName"
            :key="index"
      ...
      <b-col span="8">
        <div class="color-wrap">
          <div
            class="item"
            :class="name"
            v-for="(name,index) in successName"
            :key="index"
      ...
      <b-col span="8">
        <div class="color-wrap">
          <div
            class="item"
            :class="name"
            v-for="(name,index) in warningName"
            :key="index"
            @click="copyColor(warning[index])"
            :style="{color:index<4?'#fff':'rgba(0,0,0,.85)'}"
          >
            <span>{{ name }}</span>
            <span>{{ warning[index] }}</span>
          </div>
        </div>
      </b-col>
      <b-col span="8">
        <div class="color-wrap">
          <div
            class="item"
            :class="name"
            v-for="(name,index) in dangerName"
            :key="index"
      ...
      <b-col span="8">
        <div class="color-wrap">
          <div
            class="item"
            :class="name"
            v-for="(name,index) in cyanName"
            :key="index"
            @click="copyColor(cyan[index])"
            :style="{color:index<4?'#fff':'rgba(0,0,0,.85)'}"
          >
            <span>{{ name }}</span>
            <span>{{ cyan[index] }}</span>
          </div>
        </div>
      </b-col>
    </b-row>
